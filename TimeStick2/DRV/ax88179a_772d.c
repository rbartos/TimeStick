...
#endif
{
	...
	val &= ~MDIO_EEE_100TX;
        ...
        if (val & MDIO_EEE_1000T) {
            linkmode_set_bit(ETHTOOL_LINK_MODE_1000baseT_Full_BIT, data->supported);
        }
#else
	...
	if (val < 0)
		return val;
#if LINUX_VERSION_CODE >= KERNEL_VERSION(6, 11, 0)
        ethtool_convert_legacy_u32_to_link_mode(data->advertised, val);
#else
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(6, 11, 0)
        if (edata->advertised[0] & MDIO_EEE_100TX) {
#else
